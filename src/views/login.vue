<template>
  <div class="login-wrap">
    <a-form :model="loginForm" class="login-container">
      <h3 class="title">用户登录</h3>
      <a-form-item>
        <a-input type="text" v-model:value="loginForm.username" placeholder="账号"></a-input>
      </a-form-item>
      <a-form-item>
        <a-input type="password" v-model:value="loginForm.password" auto-complete="off" placeholder="密码"></a-input>
      </a-form-item>
      <a-checkbox-group v-model:value="loginForm.rememberMe">
        <a-checkbox class="remember" value="on">记住我</a-checkbox>
      </a-checkbox-group>
      <a-form-item style="width:100%;">
        <a-button type="primary" style="width:100%;" @click="submitForm" :loading="logining">登录</a-button>
<!--        <br/><a-button type="primary" style="width:100%;" @click="checkAuthor" :loading="check">检查auth</a-button>-->
      </a-form-item>
    </a-form>
  </div>
</template>
...
//       let loginUrl = 'http://' + this.$store.state.config.domain + 'session';
//       if(loginForm.value.rememberMe.length > 0){
//         loginUrl += '?remember-me=on';
//       }
//       // axios.post(loginUrl, JSON.stringify(loginForm.value))
//     };
//
//     return {
//       loginForm,
...
export default {
  ...
  methods: {
    submitForm(){
      ...
      // let loginUrl = 'http://' + this.$store.state.config.domain + '/api/session';
      this.logining = true;
      let loginUrl = '/api/session';
      if(this.loginForm.rememberMe.length > 0){
        loginUrl += '?remember-me=on';
      }
    ...
    checkAuthor(){
      axios.get('/api/admin/hello').then(res=>{
...
.login-wrap {
  ...
  background-image: url("../assets/img/top_img.jpeg");
